...
def _get_fernet():
    ...
    try:
        ...
        _logger.error('smart_ordering: SMART_FERNET_KEY is set but invalid: %s', e)
        raise ValueError(
...
def encrypt_secret(value):
    ...
    fernet = _get_fernet()
    if not fernet:
        _logger.warning('smart_ordering: SMART_FERNET_KEY not set, storing secret in plaintext')
        return value
